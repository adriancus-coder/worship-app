...
const DEFAULTS = {
  ...
  clock_scale: '1.8',
  // '24' or '12': every clock (the projector's and the live pages').
  time_format: '24',
  // "Margine de siguranță proiector", % of each edge kept free (overscan); a screen may override.
  safe_margin: '5',
};
const SAFE_MARGIN = { min: 0, max: 12, fallback: 5 };

// 0-12 (integer) or null when not a valid margin.
function parseSafeMargin(value) {
  const n = typeof value === 'string' && /^\d{1,2}$/.test(value.trim()) ? Number(value) : value;
  return Number.isInteger(n) && n >= SAFE_MARGIN.min && n <= SAFE_MARGIN.max ? n : null;
}

const { normalize: normalizeClock, TIME_FORMATS } = require('./clock');
...
function createAdminSettings(db) {
  ...
  // The church default safe margin, 0-12 (%).
  function safeMargin(adminId) {
    const n = parseSafeMargin(get(adminId, 'safe_margin'));
    return n === null ? SAFE_MARGIN.fallback : n;
  }

  // '24' | '12', for every clock.
  function timeFormat(adminId) {
    const value = get(adminId, 'time_format');
    return TIME_FORMATS.includes(value) ? value : DEFAULTS.time_format;
  }

  return { get, set, timezone, chordNotationDefault, themeDefault, service, clock, setClock, timeFormat, safeMargin };
}

module.exports = { DEFAULTS, NOTATIONS, THEME_DEFAULTS, TIME_FORMATS, isValidTimezone, createAdminSettings, SAFE_MARGIN, parseSafeMargin };
