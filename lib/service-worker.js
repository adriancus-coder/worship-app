...
//   deploy = a new cache; old ones are deleted when the new worker activates). Scripts and
//   styles carry the version in their URL (?v=): a page of a newer deploy, loaded while this
//   worker is still in charge, finds its files in the new worker's cache or on the network.
...
// A new worker waits until a page asks it to take over ({ type: 'SKIP_WAITING' }, public/pwa.js).
...
const PRECACHED = new Set(CONFIG.precache.map((url) => url.split('?')[0]));
...
async function cacheFirst(request) {
  const cached = (await caches.match(request, { cacheName: STATIC_CACHE })) || (await caches.match(request));
