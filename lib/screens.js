...
function createScreenStore(db) {
  ...
  const selectByHash = db.prepare(`SELECT s.*, a.name AS admin_name FROM screens s JOIN admins a ON a.id = s.admin_id
  ...
  function findByToken(token) {
    ...
    return row ? { ...toScreen(row), adminId: row.admin_id, adminName: row.admin_name } : null;
  }
