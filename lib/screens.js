...
function toScreen(row) {
  return { id: row.id, name: row.name, createdAt: row.created_at, lastSeenAt: row.last_seen_at, safeMargin: row.safe_margin === undefined ? null : row.safe_margin };
}
...
function createScreenStore(db) {
  ...
  return { startPairing, claimCode, createLink, collect, collectLink, findByToken, touch, list, get, rename, revoke, setSafeMargin, cleanup };
}
