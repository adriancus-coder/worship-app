...
function createAuth({ db, config }) {
  ...
  function isPlatformOwner(user, admin) {
    return Boolean(user && admin && user.role === 'owner' && admin.id === platformAdminId());
  }
  ...
  function getSession(req) {
    ...
    return {
      sessionId,
      user: {
        id: row.id, name: row.name, email: row.email, role: row.role, locale: row.locale, chordNotation: row.chord_notation,
        theme: row.theme,
        mustChangePassword: Boolean(row.must_change_password),
      },
      admin: { id: row.admin_id, name: row.admin_name },
      platformOwner: row.role === 'owner' && row.admin_id === platformAdminId(),
    };
  }
