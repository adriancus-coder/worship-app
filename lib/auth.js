...
function createAuth({ db, config }) {
  const insertSession = db.prepare(`INSERT INTO sessions (id, user_id, admin_id, created_at, expires_at)
    VALUES (?, ?, ?, ?, ?)`);
  const selectSession = db.prepare(`SELECT u.id, u.name, u.email, u.role, u.locale, u.chord_notation,
  ...
  function getSession(req) {
    ...
    return {
      sessionId,
      user: { id: row.id, name: row.name, email: row.email, role: row.role, locale: row.locale, chordNotation: row.chord_notation },
