...
const TEAM_ROLES = ['presenter', 'leader', 'operator', 'member']; // never 'owner': one owner per admin
...
function createTeamStore(db) {
  ...
  const listRows = db.prepare(`SELECT ${columns} FROM users WHERE admin_id = ?
    ORDER BY CASE role WHEN 'owner' THEN 0 WHEN 'presenter' THEN 1 WHEN 'leader' THEN 2 WHEN 'operator' THEN 3 ELSE 4 END, name COLLATE NOCASE, id`);
