...
function toItem(row, song, t) {
  ...
  return {
    ...
    song: row.type === 'song' && row.song_id !== null
      ...
      : null,
    songDeleted,
    // Operator additions (projector-only unless accepted into the shared setlist).
    scope: row.scope,
    request: row.request_status
      ? { status: row.request_status, by: row.requested_by_name || null, at: row.requested_at }
      : null,
    ...options,
  };
}
...
function createEventStore(db) {
  ...
  const itemsQuery = (onlyShared) => db.prepare(`SELECT i.id, i.position, i.type, i.song_id, i.title, i.body, i.reference, i.url,
      ...
      i.scope, i.request_status, i.requested_at, u.name AS requested_by_name,
    ...
    LEFT JOIN users u ON u.id = i.requested_by AND u.admin_id = i.admin_id
  ...
  const insertOperatorItem = db.prepare(`INSERT INTO setlist_items
    (event_id, admin_id, position, type, song_id, title, body, reference, url, duration_min,
      transpose, arrangement, team_note, reference_url, media_id, scope, request_status, requested_by, requested_at)
    VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?, 0, NULL, NULL, NULL, NULL, 'projector', ?, ?, ?)`);
  const selectRequest = db.prepare('SELECT id, scope, request_status FROM setlist_items WHERE id = ? AND event_id = ? AND admin_id = ?');
  const updateRequest = db.prepare('UPDATE setlist_items SET scope = ?, request_status = ? WHERE id = ? AND event_id = ? AND admin_id = ?');
  const countPending = db.prepare("SELECT COUNT(*) FROM setlist_items WHERE event_id = ? AND admin_id = ? AND request_status = 'pending'");
  ...
  const selectMediaRef = db.prepare('SELECT id, title FROM media WHERE id = ? AND admin_id = ?');
  ...
  // A projector-only item (song / verse / announcement, validated) right after afterId
  // (or at the end). request: { userId, at } when proposed to the leader. Returns its id.
  function addOperatorItem(adminId, eventId, it, afterId, request) {
    const id = Number(insertOperatorItem.run(eventId, adminId, it.type, it.songId, it.title, it.body, it.reference, it.url,
      it.durationMin, request ? 'pending' : null, request ? request.userId : null, request ? request.at : null).lastInsertRowid);
  ...
  // { id, scope, status } of an item of this event, or null.
  function requestOf(adminId, eventId, itemId) {
    const row = selectRequest.get(itemId, eventId, adminId);
    return row ? { id: row.id, scope: row.scope, status: row.request_status } : null;
  }

  function setRequest(adminId, eventId, itemId, scope, status) {
    updateRequest.run(scope, status, itemId, eventId, adminId);
  }

  function pendingRequests(adminId, eventId) {
    return countPending.pluck().get(eventId, adminId);
  }
  ...
  return {
    ...
    addOperatorItem, moveAfter, requestOf, setRequest, pendingRequests, lastShared,
  };
}
