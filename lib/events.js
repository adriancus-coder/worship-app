...
const { parseArrangement, defaultArrangement, sectionCodes, sectionLabels, firstLyricLine } = require('./sections');

const ITEM_TYPES = ['song', 'verse', 'video', 'announcement', 'sermon', 'other'];
// Statuses: planned (from creation: the team sees it at once) -> live -> finished. Only
// templates are hidden from the team.
const STATUSES = ['planned', 'live', 'finished'];
// The roles with full rights over events: create, edit, templates, delete, start,
...
// The same three roles edit everything the church runs a service with: the library
// (create, edit, delete, file import / export, the key, backgrounds), media and screens.
// Roles differ only by the page they land on (leader: live, operator: the console).
// Owner-only stays owner-only: settings, team, logo, backup, platform.
const EDITOR_ROLES = EVENT_ROLES;
// The projector screens (pair with a code, rename, revoke, "Deschide ecranul
// proiectorului"): the owner and the operator - the leader leads, the operator runs the
// projector PC. The leader keeps the preview and the source buttons.
const SCREEN_ROLES = ['owner', 'operator'];
const LIMITS = {
  nameMax: 120,
  notesMax: 2000,
  itemsMax: 60,
...
// findBackground(id) -> truthy for a background (image / loop) of the same admin.
// Every item may carry `background`: null / absent (inherit: the song's, else the church
// default), 'none' (explicitly none) or a background media id (lib/backgrounds.js).
// Returns { value: [clean items] } or { error }.
function validateItems(items, t, findSong, findMedia = () => null, findBackground = () => null) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const out = {
      ...
      transpose: 0, arrangement: null, teamNote: null, referenceUrl: null, backgroundMediaId: null, backgroundNone: 0,
    };
    const bg = item.background;
    if (bg === 'none') out.backgroundNone = 1;
    else if (bg !== undefined && bg !== null && bg !== '') {
      if (!/^\d{1,15}$/.test(String(bg))) return { error: t('errors.itemBackgroundInvalid', { n }) };
      // A background deleted meanwhile (an editor open for a while) simply inherits again.
      if (findBackground(Number(bg))) out.backgroundMediaId = Number(bg);
    }
...
function songOptions(row, song, t) {
  ...
  return {
    ...
    // Each step: its code, label and section, and the section's first lyric line (chords
    // stripped, so transpose / notation never change it) for the live step buttons.
    arrangementResolved: codes.map((code) => {
      const index = canonical.indexOf(code);
      return { code, label: labels[index], sectionId: song.sections[index].id, firstLine: firstLyricLine(song.sections[index].content) };
    }),
...
function toItem(row, song, t) {
  ...
  return {
    ...
    // projector only).
    scope: row.scope,
    // The item's own background: null (inherit), 'none' or a media id (lib/backgrounds.js).
    background: row.background_none ? 'none' : row.background_media_id || null,
    ...options,
  };
}
...
function createEventStore(db) {
  ...
  const itemsQuery = (onlyShared) => db.prepare(`SELECT i.id, i.position, i.type, i.song_id, i.title, i.body, i.reference, i.url,
      ...
      i.scope, i.background_media_id, i.background_none,
  ...
  const deleteSharedItems = db.prepare("DELETE FROM setlist_items WHERE event_id = ? AND admin_id = ? AND scope = 'shared'");
  const selectLatestTemplate = db.prepare('SELECT id FROM events WHERE admin_id = ? AND is_template = 1 ORDER BY updated_at DESC, id DESC LIMIT 1').pluck();
  ...
  const updateItem = db.prepare(`UPDATE setlist_items SET position = ?, type = ?, song_id = ?, title = ?, body = ?,
      ...
      media_id = ?, background_media_id = ?, background_none = ?
    WHERE id = ? AND event_id = ? AND admin_id = ?`);
  const insertItem = db.prepare(`INSERT INTO setlist_items
    (event_id, admin_id, position, type, song_id, title, body, reference, url, duration_min,
      transpose, arrangement, team_note, reference_url, media_id, background_media_id, background_none)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  ...
  const lastSungRows = db.prepare(`SELECT i.song_id, MAX(e.event_date) AS last_date
    ...
    WHERE i.admin_id = ? AND i.song_id IS NOT NULL AND i.scope = 'shared' AND e.is_template = 0
      AND (e.status IN ('live', 'finished') OR (e.status = 'planned' AND e.event_date < @today)) AND e.event_date <= @today
    GROUP BY i.song_id`);

  // when (status and date, never the date alone):
  //   'upcoming'   planned or live, and (date >= today OR live): a live event forgotten
  //                yesterday stays here, first; ascending by date / time
  //   'past'       finished (even today), or planned with its date passed; descending
  //   'templates'
  // teamOnly: never templates (the team sees every other event).
  function list(adminId, { when, today, teamOnly }) {
    const where = ['e.admin_id = ?'];
    const params = [adminId];
    let order = 'CASE WHEN e.status = \'live\' THEN 0 ELSE 1 END, e.event_date ASC, COALESCE(e.start_time, \'\') ASC, e.id ASC';
    if (when === 'templates') {
      where.push('e.is_template = 1');
      order = 'e.name COLLATE NOCASE ASC, e.id ASC';
    } else {
      where.push('e.is_template = 0');
      where.push(when === 'past'
        ? "(e.status = 'finished' OR (e.status = 'planned' AND e.event_date < ?))"
        : "(e.status = 'live' OR (e.status = 'planned' AND e.event_date >= ?))");
      params.push(today);
      if (when === 'past') order = 'e.event_date DESC, COALESCE(e.start_time, \'\') DESC, e.id DESC';
    }
    if (teamOnly && when === 'templates') return [];
    return db.prepare(`SELECT ${EVENT_COLUMNS} FROM events e WHERE ${where.join(' AND ')} ORDER BY ${order}`)
      .all(...params).map(toEvent);
  }

  // null when missing, or when teamOnly and the event is a template.
  ...
  function get(adminId, id, { teamOnly = false, t = (key) => key, scope = 'shared' } = {}) {
    ...
    if (!row) return null;
    if (teamOnly && row.is_template) return null;
  ...
  const selectBackgroundRef = db.prepare(`SELECT id FROM media WHERE id = ? AND admin_id = ? AND kind IN ('image', 'loop')`);
  function findBackground(adminId, mediaId) {
    return selectBackgroundRef.get(mediaId, adminId) || null;
  }
  ...
  function writeItems(adminId, eventId, items) {
    ...
    const sharedIds = items.map((it, position) => {
      const values = [position, it.type, it.songId, it.title, it.body, it.reference, it.url, it.durationMin,
        ...
        it.backgroundMediaId || null, it.backgroundNone ? 1 : 0];
      if (keepId[position]) {
  ...
  // The event's shared items replaced by another event's or a template's (the editor's
  // "Program de la": Șablon / Copie a evenimentului); the copies get new ids. -> boolean
  const applyFrom = db.transaction((adminId, id, sourceId) => {
    if (!selectEvent.get(id, adminId)) return false;
    deleteSharedItems.run(id, adminId);
    copyItems.run(id, sourceId, adminId);
    touchEvent.run(Date.now(), id, adminId);
    return true;
  });
  const applyTemplate = applyFrom;

  // "Program de la: Gol": every shared item goes. -> boolean
  const clearItems = db.transaction((adminId, id) => {
    if (!selectEvent.get(id, adminId)) return false;
    deleteSharedItems.run(id, adminId);
    touchEvent.run(Date.now(), id, adminId);
    return true;
  ...
  // The most recently changed template, or null.
  function latestTemplateId(adminId) {
    const id = selectLatestTemplate.get(adminId);
  ...
  // The (non-template) events a song is in, newest first.
  ...
  // Map songId -> the last date it was sung ("Cântată ultima dată"): a live or finished
  // event, or a planned one whose date has passed (nobody pressed "Pornește").
  function lastSungMap(adminId, today) {
    return new Map(lastSungRows.all(adminId, { today }).map((r) => [r.song_id, r.last_date]));
  }

  return {
    list, get, findSong, findMedia, findBackground, itemSong, create, updateMeta, replaceItems, setStatus, remove, songHistory, lastSungMap,
    latestTemplateId, applyTemplate, applyFrom, clearItems,
    addOperatorItem, moveAfter, lastShared,
  };
}

module.exports = { ITEM_TYPES, STATUSES, EVENT_ROLES, EDITOR_ROLES, SCREEN_ROLES, LIMITS, validateEventMeta, validateItems, createEventStore };
