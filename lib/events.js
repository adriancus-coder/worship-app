...
// items: [{ id?, type, songId, title, body, reference, url, durationMin,
...
function validateItems(items, t, findSong) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const out = {
      ...
      type: item.type, songId: null, title: null, body: null, reference: null, url: null, durationMin,
...
function createEventStore(db) {
  ...
  const selectItems = db.prepare(`SELECT i.id, i.position, i.type, i.song_id, i.title, i.body, i.reference, i.url,
      i.duration_min, i.transpose, i.arrangement, i.team_note, i.reference_url,
  ...
  const selectItemIds = db.prepare('SELECT id FROM setlist_items WHERE event_id = ? AND admin_id = ?');
  ...
  // t labels the arrangement's sections (the request's language).
  function get(adminId, id, { teamOnly = false, t = (key) => key } = {}) {
    ...
    const items = selectItems.all(id, adminId).map((item) => {
      let song = null;
  ...
  function itemSong(adminId, eventId, itemId, t) {
    const found = get(adminId, eventId, { t });
  ...
  // Items that name an existing item of this event (once) are updated in place and keep
  // their id; the others are inserted; items no longer listed are deleted.
  function writeItems(adminId, eventId, items) {
    const existing = new Set(selectItemIds.pluck().all(eventId, adminId));
    const kept = new Set();
    ...
    items.forEach((it, position) => {
      const values = [position, it.type, it.songId, it.title, it.body, it.reference, it.url, it.durationMin,
        it.transpose || 0, it.arrangement, it.teamNote, it.referenceUrl];
      if (keepId[position]) updateItem.run(...values, keepId[position], eventId, adminId);
      else insertItem.run(eventId, adminId, ...values);
    });
  }
  ...
  return { list, get, findSong, itemSong, create, updateMeta, replaceItems, setStatus, remove, songHistory, lastSungMap };
}
