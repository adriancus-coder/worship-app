...
function createEventStore(db) {
  ...
  // The event's shared items replaced by a template's (the editor's "Șablon"). -> boolean
  const applyTemplate = db.transaction((adminId, id, templateId) => {
    if (!selectEvent.get(id, adminId)) return false;
    deleteSharedItems.run(id, adminId);
    copyItems.run(id, templateId, adminId);
    touchEvent.run(Date.now(), id, adminId);
    return true;
  });
  ...
  return {
    ...
    latestTemplateId, applyTemplate,
