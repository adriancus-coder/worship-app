...
const SWEEP_AFTER_MS = 60 * 60 * 1000; // leftover temp copies older than this go at startup
...
function createBackupService({ db, dataDir, config }) {
  ...
  // Startup: temp copies left by a crash (.backup-* in DATA_DIR older than olderThanMs) are
  // removed. Returns how many. A copy still being written by a live request is younger.
  function sweepTemp(olderThanMs = SWEEP_AFTER_MS, now = Date.now()) {
    let removed = 0;
    let entries;
    try {
  ...
  return { prepare, sweepTemp, recordDownload: log.recordDownload, lastBackup: log.lastBackup };
}

module.exports = { REMIND_AFTER_MS, SWEEP_AFTER_MS, createBackupLog, createBackupService, filesUnder };
