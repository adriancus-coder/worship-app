...
//   next      the next event today or later: published (the team sees it) or, for owner /
//             leader, also a draft (its status says so)
...
function createHome(db) {
  ...
  function home(adminId, role, today) {
    const editor = EDITOR_ROLES.includes(role);
