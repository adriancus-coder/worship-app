...
function createLiveStore(db) {
  ...
  const upsertState = db.prepare(`INSERT INTO live_state
      (event_id, admin_id, version, worship_item_id, worship_step, started_at, updated_at)
    VALUES (@eventId, @adminId, @version, @itemId, @step, @startedAt, @now)
    ON CONFLICT (event_id) DO UPDATE SET version = @version, worship_item_id = @itemId,
      worship_step = @step, started_at = @startedAt, updated_at = @now`);
  ...
  function command(adminId, eventId, cmd, expectedVersion) {
    return apply(adminId, eventId, expectedVersion, ({ event, position, layout: lay }) => {
      ...
      switch (cmd.type) {
        case 'event.start':
          ...
          return { status: 'live', start: true, position: firstPosition(lay) };
        case 'event.end':
  ...
  return { visibleEvent, layout, snapshot, command, setlistChanged, liveEventsWithSongId };
}
