...
const {
  NO_POSITION, layoutOf, firstPosition, samePosition, nextPosition, prevPosition, nextItemPosition, gotoPosition, clampPosition,
...
function createLiveStore(db) {
  ...
  function command(adminId, eventId, cmd, expectedVersion, role) {
    return apply(adminId, eventId, expectedVersion, ({ event, row, mode, position, projector, video, layout: lay, projectorLayout }) => {
      ...
      if (cmd.type === 'worship.endItem') {
        if (event.status !== 'live') throw new LiveError('notLive');
        return endItem(adminId, cmd, mode, position, projector, lay, projectorLayout, (row && row.projector_source) || 'content');
      }
  ...
  // "End of this item" (event roles): the first step of the next item in the list the
  // sender drives — the main position, or with target 'projector' (split mode only) the
  // projector position. After the last item the position stays (the team still sees where
  // the service is) and the projector goes to the logo, or black without one; in split mode
  // the main list ending touches nothing (the projector is not the team's).
  function endItem(adminId, cmd, mode, worship, projector, lay, projectorLayout, source) {
    ...
    const target = nextItemPosition(onProjector ? projectorLayout : lay, pos);
    if (target) return onProjector ? { projector: target } : { position: target };
    if (!onProjector && mode === 'split') return { noop: true };
    const clear = selectLogo.get(adminId) ? 'logo' : 'black';
    return source === clear ? { noop: true } : { source: clear };
  }
