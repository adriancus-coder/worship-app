...
function createLiveStore(db) {
  ...
  // context: { event, row, position, video, layout }; change returns { position?, status?,
  // start?, source?, video? (the whole next video state) },
  ...
  const apply = db.transaction((adminId, eventId, expectedVersion, change) => {
    ...
    const result = change({ event, row, position, video, layout: layout(adminId, eventId) });
  ...
  function command(adminId, eventId, cmd, expectedVersion) {
    return apply(adminId, eventId, expectedVersion, ({ event, row, position, video, layout: lay }) => {
      ...
      switch (cmd.type) {
        case 'event.start':
          ...
          return { status: 'live', start: true, position: firstPosition(lay), source: 'content', video: { ...NO_VIDEO, seq: video.seq } };
        case 'event.end':
          if (event.status !== 'live') throw new LiveError('notLive');
