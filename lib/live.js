...
function createLiveStore(db) {
  ...
  const upsertState = db.prepare(`INSERT INTO live_state
      (event_id, admin_id, version, worship_item_id, worship_step, projector_follows, lead_mode, team_mode,
        ...
        worship_ended, projector_ended, show_clock, clock_position, clock_scale)
    VALUES (@eventId, @adminId, @version, @itemId, @step, @follows, @mode, @teamMode,
        ...
        @worshipEnded, @projectorEnded, @showClock, @clockPosition, @clockScale)
  ...
  function snapshot(adminId, eventId) {
    ...
    return {
      ...
      video: videoOf(row),
      // The corner clock (public/clock.js), with the church timezone the screens format in.
      clock: { ...clockOf(adminId, row), timeZone: settings.timezone(adminId) },
    };
  }

  // Runs change(context) in a transaction and stores the result with version + 1.
  // context: { event, row, mode, position, projector, video, clock, layout }; change returns
  // { position?, projector? ({ itemId?, step? }), mode?, teamMode?, status?, start?, source?,
  // video? (the whole next video state), clock? (the whole next clock), notice? (passed back to the caller) },
  ...
  const apply = db.transaction((adminId, eventId, expectedVersion, change) => {
    ...
    const result = change({ event, row, mode, position, projector, video, clock, layout: lay.shared, projectorLayout: lay.all });
  ...
  function command(adminId, eventId, cmd, expectedVersion, role) {
    return apply(adminId, eventId, expectedVersion, ({ event, row, mode, position, projector, video, clock, layout: lay, projectorLayout }) => {
      ...
      if (cmd.type === 'live.mode') {
        ...
        return cmd.mode === 'split'
          ? { mode: 'split', projector: { itemId: position.itemId, step: position.step, ended: position.ended } }
          : { mode: 'together', projector: {} };
      }
      ...
      switch (cmd.type) {
        case 'event.start':
          ...
          return {
            ...
            projector: { itemId: null, step: 0 }, video: { ...NO_VIDEO, seq: video.seq }, clock: settings.clock(adminId),
          };
        case 'event.end':
          if (event.status !== 'live') throw new LiveError('notLive');
