...
const { VISIBLE_TO_TEAM, createEventStore, validateItems } = require('./events');
...
// Operator additions: item types and the limit of requests waiting for the leader.
...
//   operator       projector position, source and video, only in operator mode; adding
//                  projector-only items (and proposing them) at any time
//   member         nothing
function permission(role, type, follows) {
  if (!role) return null;
  if (type === 'operator.addItem') return ['owner', 'leader', 'operator'].includes(role) ? null : 'forbidden';
...
function createLiveStore(db) {
  ...
  function items(adminId, eventId, scope = 'shared') {
    const found = events.get(adminId, eventId, { scope });
  ...
  // Operator additions proposed to the leader, oldest first.
  function requests(adminId, eventId) {
    return items(adminId, eventId, 'all').filter((it) => it.request).map((it) => ({
      itemId: it.id,
      type: it.type,
      title: it.title || it.reference || '',
      key: it.displayKey || null,
      status: it.request.status,
      requestedBy: it.request.by,
      requestedAt: it.request.at,
    })).sort((a, b) => a.requestedAt - b.requestedAt || a.itemId - b.itemId);
  }
  ...
  const apply = db.transaction((adminId, eventId, expectedVersion, change) => {
    ...
    const result = change({ event, row, position, projector, video, layout: lay.shared, projectorLayout: lay.all });
  ...
  // the same transaction); none for the server's own events. userId: the sender (requests).
  // Returns { version, changed }; throws LiveError.
  function command(adminId, eventId, cmd, expectedVersion, role, userId = null) {
    return apply(adminId, eventId, expectedVersion, ({ event, row, position, projector, video, layout: lay, projectorLayout }) => {
      ...
      if (cmd.type === 'operator.addItem' || cmd.type.startsWith('request.')) {
        if (event.status !== 'live') throw new LiveError('notLive');
        return itemCommand(adminId, eventId, cmd, position, projector, userId);
      }
  ...
  // Operator additions and the leader's answers. The projector and worship positions stay
  // where they are (items are followed by id).
  function itemCommand(adminId, eventId, cmd, worship, projector, userId) {
    if (cmd.type === 'operator.addItem') {
      ...
      const propose = cmd.propose === true;
      if (propose && events.pendingRequests(adminId, eventId) >= MAX_PENDING_REQUESTS) throw new LiveError('tooManyRequests');
      // Right after what the projector shows now.
      const after = projector.follows === 'operator' ? projector.itemId : worship.itemId;
      events.addOperatorItem(adminId, eventId, value[0], after, propose ? { userId, at: Date.now() } : null);
      return {};
    }
    const request = Number.isInteger(cmd.itemId) ? events.requestOf(adminId, eventId, cmd.itemId) : null;
    if (!request || request.scope !== 'projector' || request.status !== 'pending') throw new LiveError('requestNotFound');
    if (cmd.type === 'request.refuse') {
      events.setRequest(adminId, eventId, cmd.itemId, 'projector', 'refused');
      return {};
    }
    if (cmd.type !== 'request.accept' || !['afterCurrent', 'end'].includes(cmd.position)) throw new LiveError('badCommand');
    // After the item worship is on, or after the last shared item.
    const anchor = cmd.position === 'afterCurrent' && worship.itemId !== null ? worship.itemId : events.lastShared(adminId, eventId);
    events.setRequest(adminId, eventId, cmd.itemId, 'shared', 'accepted');
    events.moveAfter(adminId, eventId, cmd.itemId, anchor);
    return {};
  }
  ...
  return { visibleEvent, layouts, items, requests, snapshot, command, setlistChanged, liveEventId, liveEventsWithSongId };
}
