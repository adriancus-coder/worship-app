...
//   owner, leader, operator (EVENT_ROLES)  everything; the projector position only in
//                                          operator mode
//   member                                 nothing
function permission(role, type, follows) {
  if (!role) return null;
  if (!EVENT_ROLES.includes(role)) return 'forbidden';
  return PROJECTOR_COMMANDS.includes(type) && follows !== 'operator' ? 'notOperatorMode' : null;
}
